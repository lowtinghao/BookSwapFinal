...
export class ExchangeRequestsDAO {
    ...
    createExchangeRequest(exchangeRequest) {
        const transaction = this.db.transaction(() => {
            ...
            const requesteeListing = getListingStatement.get(exchangeRequest.requestee_listing_id);
            const requesterListing = getListingStatement.get(exchangeRequest.requester_listing_id);
            ...
            const result = insertStatement.run(exchangeRequest.requestee_listing_id, exchangeRequest.requester_listing_id, exchangeRequest.status || 'pending', requestDate);
    ...
    getExchangeRequestById(requestId) {
        ...
        const request = statement.get(requestId);
        if (!request) {
     ...
     */
    getExchangeRequestsByRequester(listingId) {
     ...
     */
    getExchangeRequestsByUser(userId) {
     ...
     */
    getExchangeRequestsByStatus(status) {
     ...
     */
    getAcceptedExchangeRequestsByUser(userId) {
        const statement = this.db.prepare(`
            SELECT er.* FROM exchange_requests er
            JOIN book_listings bl1 ON er.requestee_listing_id = bl1.listing_id
            JOIN book_listings bl2 ON er.requester_listing_id = bl2.listing_id
     ...
     */
    updateExchangeRequest(requestId, updates) {
        const transaction = this.db.transaction(() => {
            try {
                let updateFields = [];
                let updateValues = [];
     ...
     * @throws {Error} If there is a database error during deletion
     */
    deleteExchangeRequestsByListing(listingId) {
        const statement = this.db.prepare(`
     ...
     */
    exchangeRequestExists(requesteeListingId, requesterListingId) {
        const statement = this.db.prepare(`
            SELECT COUNT(*) as count FROM exchange_requests 
            WHERE requestee_listing_id = ? AND requester_listing_id = ?
        `);
        const result = statement.get(requesteeListingId, requesterListingId);
